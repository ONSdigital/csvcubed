Feature: Behaviour testing of csvcubed inspect.
    ...
    Scenario: inspect should produce the expected printable for data cube metadata json-ld input of type multi-unit single-measure.
        Given the existing test-case file "cli/inspect/multi-unit_single-measure/new/multi-unit-single-measure-dataset.csv-metadata.json"
        And the existing test-case file "cli/inspect/multi-unit_single-measure/new/multi-unit-single-measure-dataset.csv"
        And the existing test-case file "cli/inspect/multi-unit_single-measure/new/region.table.json"
        And the existing test-case file "cli/inspect/multi-unit_single-measure/new/year.table.json"
        And the existing test-case file "cli/inspect/multi-unit_single-measure/new/region.csv-metadata.json"
        And the existing test-case file "cli/inspect/multi-unit_single-measure/new/year.csv-metadata.json"
        When the Metadata file path is detected and validated "cli/inspect/multi-unit_single-measure/new/multi-unit-single-measure-dataset.csv-metadata.json"
        And the Metadata File json-ld is loaded to a rdf graph
        And the Metadata File is validated
        And the Printables for data cube are generated
        Then the Type Printable should be "- This file is a data cube."
        ...
        """
        - The data cube has the following catalog metadata:
          - Title: Multi unit single measure dataset
          - Label: Multi unit single measure dataset
          - Issued: 2023-06-22T11:21:04.071843
          - Modified: 2023-06-22T11:21:04.071843
          - License: None
          - Creator: None
          - Publisher: None
          - Landing Pages: None
          ...
          - Keywords: None
          - Contact Point: None
          - Identifier: Multi unit single measure dataset
          - Comment: None
          - Description: None
        """
        And the Data Structure Definition Printable should be
        """
        - The data cube has the following column component information:
            - Dataset Label: Multi unit single measure dataset
            - Columns:
                                   Title         Type  Required                                                Property URL Observations Column Titles
                                     Year    Dimension     True        multi-unit-single-measure-dataset.csv#dimension/year
                                   Region    Dimension     True      multi-unit-single-measure-dataset.csv#dimension/region
                                  Measure     Measures     True                http://purl.org/linked-data/cube#measureType                      Value
                                     Unit        Units     True http://purl.org/linked-data/sdmx/2009/attribute#unitMeasure
                                    Value Observations     True    multi-unit-single-measure-dataset.csv#measure/{+measure}
            - Columns where suppress output is true: None
        """
        And the Code List Printable should be
        """
