Feature: Test the creation of the code-list CSVW with only a code-list-config.json file.
    ...
    Scenario: Successfully output a code-list CSVW using schema v1.2
        Given the existing test-case file "readers/code-list-config/v1.2/code_list_config_hierarchical_v1_2.json"
        Then a valid code-list is created and serialised to CSVW from the config file "readers/code-list-config/v1.2/code_list_config_hierarchical_v1_2.json"
