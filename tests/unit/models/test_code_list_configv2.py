...
from csvcubed.models.codelistconfig.code_list_configv2 import (
    CODE_LIST_CONFIG_V2_DEFAULT_URL,
    CodeListConfigConceptV2,
    CodeListConfigV2,
)
...
def _assert_code_list_config_concepts(
    concepts: List[CodeListConfigConceptV2], concepts_jsons: List[Dict]
):
    """
        ...
        for concept_json in concepts_jsons:
            if concept.notation == concept_json["notation"]:
                ...
                if concept.exact_match:
                    assert concept.exact_match == concept_json["exact_match"]
                if concept.children:
...
def _assert_code_list_concept_sorting(
    concepts: List[CodeListConfigConceptV2], expected_sort_orders: Dict
):
    """
...
def _assert_code_list_config_data(
    code_list_config: CodeListConfigV2, code_list_config_json: Dict
):
    """
...
def test_code_list_config_without_schema():
    ...
    assert code_list_config.schema == CODE_LIST_CONFIG_V2_DEFAULT_URL
    _assert_code_list_config_data(code_list_config, code_list_config_json)
    _assert_code_list_config_concepts(
        code_list_config.concepts, code_list_config_json["concepts"]
    )
...
def test_exact_match_field_in_output_csv():
    """
