...
from .resource import NewMetadataResource, map_str_to_en_literal, map_resource_to_uri, Resource as RdfResource
...
class Resource(NewMetadataResource):
    ...
    themes: Ann[Set[str], Triple(DCAT.theme, PropertyStatus.recommended, URIRef)]  # skos:Concept
    ...
    keywords: Ann[Set[str], Triple(DCAT.keyword, PropertyStatus.recommended, map_str_to_en_literal)]
    landing_page: Ann[Optional[str], Triple(DCAT.landingPage, PropertyStatus.recommended, URIRef)]
    ...
    rights: Ann[Optional[str], Triple(DCTERMS.rights, PropertyStatus.recommended, URIRef)]
    has_policy: Ann[Optional[str], Triple(ODRL2.hasPolicy, PropertyStatus.recommended, URIRef)]
    is_referenced_by: Ann[Optional[str], Triple(DCTERMS.isReferencedBy, PropertyStatus.recommended, URIRef)]
...
class CatalogRecord(NewMetadataResource):
    ...
    modified: Ann[datetime, Triple(DCTERMS.modified, PropertyStatus.recommended, Literal)]
    primary_topic: Ann[RdfResource[Resource], Triple(FOAF.primaryTopic, PropertyStatus.mandatory, map_resource_to_uri)]
...
class Catalog(Dataset):
    ...
    records: Ann[Set[RdfResource[CatalogRecord]], Triple(DCAT.record, PropertyStatus.recommended, map_resource_to_uri)]
