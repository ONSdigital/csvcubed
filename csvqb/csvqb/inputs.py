...
def pandas_input_to_columnar(maybe_columnar_data: PandasDataTypes, allow_none: bool = True) -> Optional[pd.Series]:
    if maybe_columnar_data is None:
        ...
        raise Exception(f"Input data has shape {maybe_columnar_data.shape}. A single column of data must be provided.")
...
def pandas_input_to_columnar_str(maybe_columnar_data: PandasDataTypes, allow_none: bool = True) -> Iterable[str]:
