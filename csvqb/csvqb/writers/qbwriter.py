...
class QbWriter(WriterBase):
        ...
        for column in self.cube.columns:
            if isinstance(column, QbColumn):
                if isinstance(column.component, QbObservationValue):
                    virtual_columns += self._generate_virtual_columns_for_obs_val(column.component)

        return virtual_columns

    def _generate_virtual_columns_for_obs_val(self, obs_val: QbObservationValue) -> List[Dict[str, Any]]:
    ...
    def _generate_qb_dataset_dsd_definitions(self):
        ...
        for column in self.cube.columns:
            if isinstance(column, QbColumn):
                component_specs_for_col = self._get_qb_component_specs_for_col(column.uri_safe_identifier,
                                                                               column.component)
    ...
    def _get_qb_component_specs_for_col(self,
                                        column_name_uri_safe: str,
                                        component: QbDataStructureDefinition) -> Iterable[qb.ComponentSpecification]:
        if isinstance(component, QbDimension):
            return [self._get_qb_dimension_specification(column_name_uri_safe, component)]
        elif isinstance(component, QbAttribute):
            return [self._get_qb_attribute_specification(column_name_uri_safe, component)]
    ...
    def _get_qb_units_column_specification(self, column_name_uri_safe: str) -> qb.AttributeComponentSpecification:
        component = qb.AttributeComponentSpecification(self._doc_rel_uri(f"component/{column_name_uri_safe}"))
    ...
    def _get_qb_obs_val_specifications(self, observation_value: QbObservationValue) -> \
            List[qb.ComponentSpecification]:
        specs: List[qb.ComponentSpecification] = []

        if observation_value.unit is not None:
            unit_uri_safe_identifier = self._get_unit_uri_safe_identifier(observation_value.unit)
            specs.append(self._get_qb_units_column_specification(unit_uri_safe_identifier))

        if isinstance(observation_value, QbSingleMeasureObservationValue):
            specs.append(self._get_qb_measure_component_specification(observation_value.measure))
    ...
    def _get_qb_measure_dimension_specifications(self, measure_dimension: QbMultiMeasureDimension) -> \
    ...
    def _get_qb_measure_component_specification(self, measure: QbMeasure) -> qb.MeasureComponentSpecification:
        if isinstance(measure, ExistingQbMeasure):
            ...
            component_uri = self._doc_rel_uri(f"component/{get_last_uri_part(measure.measure_uri)}")
            ...
            component = qb.MeasureComponentSpecification(self._doc_rel_uri(f"component/{measure.uri_safe_identifier}"))
            component.measure = qb.MeasureProperty(self._doc_rel_uri(f"measure/{measure.uri_safe_identifier}"))
    ...
    def _get_qb_dimension_specification(self,
                                        column_name_uri_safe: str,
                                        dimension: QbDimension) -> qb.DimensionComponentSpecification:
        if isinstance(dimension, ExistingQbDimension):
            component = qb.DimensionComponentSpecification(self._doc_rel_uri(f"component/{column_name_uri_safe}"))
            ...
            component.dimension = qb.DimensionProperty(self._doc_rel_uri(f"dimension/{dimension.uri_safe_identifier}"))
            ...
            if dimension.code_list is not None:
                component.dimension.code_list = self._get_code_list_resource(dimension.code_list)
    ...
    def _get_code_list_resource(self, code_list: QbCodeList) -> Resource[skos.ConceptScheme]:
    ...
    def _get_qb_attribute_specification(self,
                                        column_name_uri_safe: str,
        ...
        if isinstance(attribute, ExistingQbAttribute):
            ...
            component.attribute = qb.AttributeProperty(self._doc_rel_uri(f"attribute/{attribute.uri_safe_identifier}"))
    ...
    def _define_csvw_column_for_qb_column(self, csvw_col: dict, column: QbColumn) -> None:
        (property_url, default_value_url) = self._get_default_property_value_uris_for_column(column)
    ...
    def _get_default_property_value_uris_for_multi_units(self,
                                                         column: QbColumn,
                                                         multi_units: QbMultiUnits) -> Tuple[str, str]:
    ...
    def _get_default_property_value_uris_for_multi_measure(self,
                                                           column: QbColumn,
    ...
    def _get_default_property_value_uris_for_column(self, column: QbColumn) -> \
        ...
        if isinstance(column.component, QbDimension):
            ...
            return self._get_default_property_value_uris_for_multi_units(column, column.component)
        elif isinstance(column.component, QbMultiMeasureDimension):
            return self._get_default_property_value_uris_for_multi_measure(column, column.component)
        elif isinstance(column.component, QbObservationValue):
            return None, None
        else:
            raise Exception(f"Unhandled component type {type(column.component)}")

    def _get_default_property_value_uris_for_dimension(self, column: QbColumn[QbDimension]) -> Tuple[str, Optional[str]]:
        dimension = column.component
        if isinstance(dimension, ExistingQbDimension):
            return dimension.dimension_uri, self._get_column_uri_template_fragment(column)
        elif isinstance(dimension, NewQbDimension):
            local_dimension_uri = self._doc_rel_uri(f"dimension/{dimension.uri_safe_identifier}")
            value_uri = self._get_column_uri_template_fragment(column)
            if dimension.code_list is not None:
                value_uri = self._get_default_value_uri_for_code_list_concepts(column, dimension.code_list)

            return local_dimension_uri, value_uri
        else:
            raise Exception(f"Unhandled dimension type {type(dimension)}")

    def _get_default_property_value_uris_for_attribute(self, column: QbColumn[QbAttribute]) -> Tuple[str, str]:
        attribute = column.component
        if isinstance(attribute, ExistingQbAttribute):
            return attribute.attribute_uri, self._get_column_uri_template_fragment(column)
        elif isinstance(attribute, NewQbAttribute):
    ...
    def _get_column_uri_template_fragment(self, column: CsvColumn, escape_value: bool = False) -> str:
        if escape_value:
    ...
    def _get_default_value_uri_for_code_list_concepts(self, column: CsvColumn, code_list: QbCodeList) -> str:
        column_uri_fragment = self._get_column_uri_template_fragment(column)
        if isinstance(code_list, ExistingQbCodeList):
            external_match = self.external_code_list_pattern.match(code_list.concept_scheme_uri)
            local_match = self.dataset_local_code_list_pattern.match(code_list.concept_scheme_uri)
            if external_match:
            ...
            return self._doc_rel_uri(f"concept/{code_list.metadata.uri_safe_identifier}/{column_uri_fragment}")
        else:
