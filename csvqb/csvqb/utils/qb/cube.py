    ...
    for c in cube.columns:
        if isinstance(c, QbColumn) and isinstance(c.component, ExistingQbDimension):
            if c.output_uri_template is None:
                errors.append(
                    ValidationError(
                        f"'{c.csv_column_title}' - an ExistingQbDimension must have an output_uri_template "
                        "defined."
                    )
                )
...
def _validate_attributes(cube: Cube) -> List[ValidationError]:
    ...
    for c in cube.columns:
        if isinstance(c, QbColumn) and isinstance(c.component, ExistingQbAttribute):
            if c.output_uri_template is None:
                errors.append(
                    ValidationError(
                        f"'{c.csv_column_title}' - an ExistingQbAttribute must have an output_uri_template "
                        "defined."
                    )
                )
...
def _validate_observation_value_constraints(cube: Cube) -> List[ValidationError]:
    ...
    if len(multi_units_columns) > 1:
        errors.append(
            ValidationError(
                f"Found {len(multi_units_columns)} units columns. Expected maximum 1."
            )
        )
    ...
    if len(observed_value_columns) != 1:
        errors.append(
            ValidationError(
                f"Found {len(observed_value_columns)} observation value columns. Expected 1."
            )
        )
    else:
        ...
        if len(multi_measure_obs_val_columns) == 1:
            ...
            errors += _validate_observation_value(
                single_measure_obs_val_columns[0], multi_units_columns
            )
...
def _validate_multi_measure_cube(
    ...
    if len(multi_measure_columns) == 0:
        errors.append(
            ValidationError("No multi-measure column found in multi-measure cube.")
        )
    elif len(multi_measure_columns) > 1:
        errors.append(
            ValidationError(
                f"Found {len(multi_measure_columns)} measure dimension columns defined. Expected 1."
            )
        )
...
def _validate_single_measure_cube(
    ...
    if len(multi_measure_columns) > 0:
        errors.append(
            ValidationError(
                f"Found {len(multi_measure_columns)} measure dimension columns in single measure cube."
            )
        )
